...
export function battlepass(player: Player): void {
  new ActionFormData()
    .title("§5Nexus§fSMP §8- §eBattle Pass")
...
function openBattlePass(player: Player, type: number): void {
  ...
  new ModalFormData()
    ...
    .then((res) => {
      ...
      advancedRelay("1397332229901914243", `Player ${player.name} claimed reward: ${reward.description}`, "Battle Pass Reward Claimed", "00FF00");
  ...
  function givexp(player: Player, amount: number): void {
    ...
    advancedRelay("1397332229901914243", `Player ${player.name} gained ${amount} XP. Total XP: ${amount + totalXp}`, "XP Gained", "00FF00");
  }
