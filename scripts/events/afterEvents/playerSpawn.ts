...
async function showWelcomeMenu(player: Player): Promise<void> {
    try {
        ...
        form.body(`§7Please consider checking out what you can get for §ddonating §7to §5Nexus§fSMP! §7Every donation helps us §6keep the server running §7and §aimproves your experience!\n `);
...
async function handlePlayerSpawn(player: Player): Promise<void> {
    try {
        ...
        if (player.hasTag(CRABTAGS.joined)) {
            ...
            player.sendMessage(`§aWelcome back to §5Nexus§fSMP§a, §e${player.name}!`);
        }
