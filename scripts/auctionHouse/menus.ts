...
export function auctionHouseMenu(player: Player) {
  const menu = new ActionFormData()
    .title("§5Nexus§fSMP §8- §eAuction House")
...
function listItemMenu(player: Player) {
  ...
  const menu = new ModalFormData().title("§5Nexus§fSMP §8- §eList Item").dropdown("§aSelect Item", itemList, { defaultValueIndex: 0 }).textField("§6Starting Bid", "0", { defaultValue: "0" });
...
function confirmListAuction(player: Player, item: ItemStack, startingBid: number, itemSlot: number) {
  const confirmMenu = new ModalFormData().title("§5Nexus§fSMP §8- §eConfirm Listing").toggle(`§6List ${formatItemId(item.typeId)} x${item.amount} for §a${startingBid}§6?`, { defaultValue: false });
  ...
  confirmMenu.show(player as any).then((response) => {
    ...
    if (confirmListing) {
      ...
      advancedRelay("1397332742781272094", `Auction for ${formatItemId(item.typeId)} by ${player.name} listed for $${startingBid}.`, "New Auction Listed", "00FF00");
...
function viewAuctionsMenu(player: Player, page: number = 0) {
  ...
  const menu = new ActionFormData().title("§5Nexus§fSMP §8- §eAuctions").body(`§aPage ${page + 1} of ${Math.ceil(auctions.length / auctionsPerPage)}`);
...
function showAuctionDetailsMenu(player: Player, auction: AuctionItem) {
  ...
  const menu = new ActionFormData()
    .title(`§5Nexus§fSMP §8- §eItem Info`)
...
function bidOnAuctionMenu(player: Player, auction: AuctionItem) {
  const menu = new ModalFormData().title("§5Nexus§fSMP §8- §eBid on Auction").textField("§6Bid Amount", "0", { defaultValue: "0" });
...
function confirmBid(player: Player, auction: AuctionItem, bidAmount: number) {
  const confirmMenu = new ModalFormData().title("§5Nexus§fSMP §8- §eConfirm Bid").toggle("§6Are you sure you want to place this bid?", { defaultValue: false });
  ...
  confirmMenu.show(player as any).then((response) => {
    ...
    if (confirmBid) {
      ...
      advancedRelay("1397332742781272094", `Player ${player.name} placed a bid of $${bidAmount} on auction for ${formatItemId(auction.itemId)}.`, "New Bid Placed", "00FF00");
      player.playSound(SOUNDS.Success);
    } else {
