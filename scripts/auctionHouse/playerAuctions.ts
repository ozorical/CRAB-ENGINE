...
export function myAuctionsMenu(player: Player, page: number = 0) {
  ...
  const menu = new ActionFormData().title("§5Nexus§fSMP §8- §eAuction Status").body(`§6Track §7and §breturn §7your auctions that you have listed!\n\n§aPage ${page + 1} of ${Math.ceil(myAuctions.length / auctionsPerPage)}`);
...
function confirmEndAuction(player: Player, auction: AuctionItem) {
  const confirmMenu = new ModalFormData().title("§5Nexus§fSMP §8- §eEnd Auction").toggle("§6Are you sure you want to end this auction?", { defaultValue: false });
...
function endAuctionMenu(player: Player, auction: AuctionItem) {
  if (auction.highestBidder) {
    ...
    advancedRelay("1397332742781272094", `Auction for ${formatItemId(auction.itemId)} by ${auction.seller.name} ended with highest bidder ${auction.highestBidder.name} for $${auction.currentBid}.`, "Auction Ended", "00FF00");
  } else {
    ...
    advancedRelay("1397332742781272094", `Auction for ${formatItemId(auction.itemId)} by ${auction.seller.name} ended with no bids. Item returned.`, "Auction Ended", "FF0000");
  }
