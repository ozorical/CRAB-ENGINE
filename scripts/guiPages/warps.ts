...
export let config = {
  warmUpTime: 3,
  warpLocations: [
    /*0*/ { name: "§l§fSpawn", description: "§fGo back to the lobby", icon: `textures/items/nether_star.png`, location: { x: -36, y: 50, z: 20 }, dimension: `overworld`, wild: false, showUI: true },
...
export function warpMenu(player: Player, chat?: boolean, i?: number) {
  let warpMenuForm = new ActionFormData().title("§5Nexus§fSMP §8- §eWarps");
...
function freePlayForm(player: Player) {
  let freePlayOptions = new ActionFormData()
    ...
    .then((res) => {
      if (res.selection == 0) {
        playSoundTo(player, "RandomPop");
        player.sendMessage("§aNexus-Engine is generating a random location... Please wait");
