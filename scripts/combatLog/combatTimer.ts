...
world.afterEvents.entityHurt.subscribe((hurt) => {
  ...
  if (!hurtPlayer.hasTag("combat")) {
    ...
    advancedRelay("1396650952848703578", `${attackerPlayer.name} has put ${hurtPlayer.name} in combat log`, "Player in Combat", "FF0000");
  }
  ...
  players.forEach((player) => {
    if (getScore(player, "clog")! > 0) {
      if (getScore(player, "clog") == 1 && player.hasTag("combat")) {
        ...
        advancedRelay("1396650952848703578", `${player.name} is no longer in combat`, "Player out of Combat", "4DFF00");
