import { world, Player, Vector3, system, EntityHitEntityAfterEvent, PlayerPermissionLevel } from "@minecraft/server";
...
export function hasClaimAccess(player: Player, claim: any): boolean {
  ...
  return claim.owner === player.name || (claim.sharedWith && claim.sharedWith.includes(player.name)) || player.playerPermissionLevel == PlayerPermissionLevel.Operator;
}
...
export function unclaimLand(player: Player, location: Vector3, claims: ClaimData): boolean {
  const claim = getClaimAtLocation(location, claims);
  if (!claim || (claim.owner !== player.name && player.playerPermissionLevel != PlayerPermissionLevel.Operator)) {
    playSoundTo(player, "Error");
    return false;
  }
  ...
  if (scoreboard) {
    ...
    relay(`[Homes] ${player.name} refunded ${refundAmount} for unclaiming land.`);
  }
...
export async function showEnhancedLandMenu(player: Player): Promise<void> {
  ...
  const form = new ActionFormData().title("§5Nexus§fSMP §8- §eLand Claim").body(`Location: §b${Math.floor(location.x)}, ${Math.floor(location.z)}`);
  ...
  if (isClaimed) {
    ...
    if (landInfo && (landInfo.owner === player.name || player.playerPermissionLevel == PlayerPermissionLevel.Operator)) {
...
async function showClaimInfoMenu(player: Player) {
  const form = new ActionFormData().title("§5Nexus§fSMP §8- §eClaim Info").body(`§bWelcome to the land claim system! This allows you to protect your builds and areas from other players.
...
async function showShareClaimMenu(player: Player, claim: any, claims: ClaimData) {
  ...
  if (action === "add") {
    ...
    relay(`[Homes] ${claim.owner} shared their claim with ${playerName}`)
  } else {
    if (!claimData.sharedWith.includes(playerName)) {
      playSoundTo(player, "Error");
...
async function showClaimSettingsMenu(player: Player, claim: any, claims: ClaimData) {
  const form = new ModalFormData()
    .title("§5Nexus§fSMP §8- §eClaim-Config")
