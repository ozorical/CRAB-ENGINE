...
async function teleportToHomesMenu(player: Player) {
  let homesDB = await getPlayerDB(player);
  const form = new ActionFormData().title("§cCrab§fSMP §8- §eTeleport to Homes").body("§7Select a home to teleport to.");
...
async function deleteHomesMenu(player: Player) {
  let homesDB = await getPlayerDB(player);
  const form = new ActionFormData().title("§cCrab§fSMP §8- §eDelete Homes").body("§7Select a home to delete.");
...
async function shareHomesMenu(player: Player) {
  let homesDB = await getPlayerDB(player);
  const form = new ActionFormData().title("§cCrab§fSMP §8- §dShare Homes").body("§7Select a home to share.");
